...
| **Deep links de invitación (UC-03)** | `POST /patients/:id/invitations` devuelve `{ token, link, expiresAt }` (el `link` se arma con `APP_BASE_URL` del ambiente → `https://keru.ar/invite/:token` en prod, `https://dev.keru.ar/invite/:token` en dev; nunca hardcodeado). La app abre ese deep link, muestra el preview con `GET /invitations/:token`, y confirma con `POST /invitations/:token/confirm` (requiere estar logueado como el invitado). Vence a los **30 min**, un solo uso. |
